...
const Pagination = ({ data = []}) => {
  const {area1,minPrice,maxPrice,minAcreage,maxAcreage} = useSelector((state:any) => state.search)
  ...
  const checkSearch = (arr:any ,postLimit:any)=>{
      ...
      const filteredArr = arr.filter((item:any) => {
        ...
        return Object.keys(postLimit).every(key => {
          return postLimit[key] === item[key] || (item.attributes && postLimit[key] === item.attributes[key]);
        });
      });
