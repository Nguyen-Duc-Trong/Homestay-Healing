          ...
          className='w-[700px] top-[70px] bg-[#fff] max-h-[500px] absolute rounded-[8px]'>
            ...
            <div className='pt-[10px] pb-[35px] px-[25px] flex flex-col'>
              {content?.map((item:any) => {
                const newLocal = <input type='radio' name={name} id={item.code} value={item.code} checked={item?.code === queries[`${name}Code`] ? true : false} onClick={(e) => handleSubmit(e, { [name]: item.value, [`${name}Code`]: item.code })} />
                return(
