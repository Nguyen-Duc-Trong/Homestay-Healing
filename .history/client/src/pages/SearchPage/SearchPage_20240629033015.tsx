...
const SearchPage = () => {
  ...
  const { province, category, area, price } = useSelector((state: any) => state.search);
  ...
  return (
    <React.Fragment>
      <div className='h-[55px] flex p-[10px] bg-[#febb02] rounded-[8px] items-center justify-around text-[13px] '>
        ...
        <span onClick={createHandleShowModal(area, 'area')} className='flex-1 mx-[3px]'>
          <InputSearch IconBefore={<SlCrop />} IconAfter={<MdOutlineNavigateNext />} text={
            // @ts-ignore
            queries.area2} defaultText={'Chọn diện tích'} fontWeight={undefined} />
        </span>
