    ...
    <React.Fragment>
      <div className='h-[55px] flex p-[10px] bg-[#febb02] rounded-[8px] items-center justify-around text-[13px] '>
        <span onClick={createHandleShowModal(category, 'category')} className='flex-1 mx-[3px]'>
          <InputSearch IconBefore={<FaRegBuilding />} IconAfter={<LuDelete />} text={queries.category || "Loại" } fontWeight />
        </span>
        <span onClick={createHandleShowModal(province, 'province')} className='flex-1 mx-[3px]'>
          <InputSearch IconBefore={<TfiLocationPin />} IconAfter={<MdOutlineNavigateNext />} text={queries.province ||'Địa chỉ'} fontWeight={undefined} />
        </span>
        <span onClick={createHandleShowModal(price, 'price')} className='flex-1 mx-[3px]'>
          <InputSearch IconBefore={<RiMoneyDollarCircleLine />} IconAfter={<MdOutlineNavigateNext />} text={queries.price || 'Chọn giá'} fontWeight={undefined} />
        </span>
        <span onClick={createHandleShowModal(area2, 'area2')} className='flex-1 mx-[3px]'>
          <InputSearch IconBefore={<SlCrop />} IconAfter={<MdOutlineNavigateNext />} text={queries.area2 || 'Chọn diện tích'} fontWeight={undefined} />
        </span>
