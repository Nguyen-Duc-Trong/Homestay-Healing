  ...
  return (
    <React.Fragment>
      <div className='h-[55px] flex p-[10px] bg-[#febb02] rounded-[8px] items-center justify-around text-[13px] '>
        <span onClick={() => handleShowModal('Phòng trọ')} className='flex-1 mx-[3px]'>
          <InputSearch IconBefore={<FaRegBuilding />} IconAfter={<LuDelete />} text={'Loại'} fontWeight />
        </span>
        <span onClick={() => handleShowModal('Toàn quốc')} className='flex-1 mx-[3px]'>
      ...
      {isShowModal && <Modal setIsShowModal={setIsShowModal} content={undefined}/> } 
    </React.Fragment>
  )
}
