...
import { apiAddBookRoom } from '../../service/bookRoom.js';
...
const DetailPost = () => {
  ...
  const {id , images, address, attributes, star, description, users, Overviews } = location.state
...
const handleAdd = async ()=>{
  console.log({userId : data.id , postId :id});
  
  // const response = apiAddBookRoom({userId : data.id , postId :id})
    ...
    <div className='flex w-full'>
      ...
      <div className='w-[30%] pl-[15px]'>
          <div className='bg-[#febb02] border-[1px] rounded-[5px] border-[#febb02] p-[15px]'>
            ...
            <Button text={'Thêm vào giỏ phòng'} onClick={handleAdd} className={'w-full h-[40px] bg-[#fff] font-bold text-[#000000] text-[14px] mb-[10px]'} icon={<GoHeartFill size={20} color='#f73859' />} bgColor={undefined} textColor={undefined} px={undefined}/>
            <Button text={'Đặt Phòng'} onClick={undefined} className={'w-full h-[40px] bg-[#fff] font-bold text-[#000000] text-[14px]'} icon={<PiShoppingCart/>} bgColor={undefined} textColor={undefined} px={undefined}/>
          </div>
      </div>
