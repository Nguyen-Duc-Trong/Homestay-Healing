...
const PopupBookRoom = ({ setIsShowBookRoom, item, handleSubmit }) => {
    ...
    const handleAccept = async (e:any) => {
        ...
        const bookingDate = new Date().toISOString().split('T')[0]; // Lấy ngày hiện tại
        setPayload({ ...payload, bookingDate });
        setIsShowBookRoom(false)
        await apiAddContract( {userId: id, dataContract: {...payload, totalCost: totalCost}})
    ...
    return (
        <div onClick={() => { setIsShowBookRoom(false) }} className='fixed top-[0px] left-0 right-0 h-screen w-[100%] z-[1000] bg-[#00000080] flex justify-center '>
            <div onClick={(e) => {
                ...
                <div>
                    <p> - Họ tên người đặt: <span className='font-bold'>{username}</span></p>
                    <p> - Số điện thoại: <span className='font-bold'>{phone}</span></p>
                    ...
                    <p> - Tổng tiền cọc (50%): {totalCost.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</p>
                </div>
