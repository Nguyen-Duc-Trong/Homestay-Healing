        ...
        <div onClick={() => { setIsShowBookRoom(false) }} className='fixed top-[0px] left-0 right-0 h-screen w-[100%] z-[1000] bg-[#00000080] flex justify-center '>
            <div onClick={(e) => {
                ...
                <div className='px-[20px] pt-[20px]'>
                    <p className='my-[5px]'> <span className='font-bold'>- Họ tên người đặt: </span><span className='text-red-500'>{username}</span></p>
                    <p className='my-[5px]'> <span className='font-bold'>- Số điện thoại: </span>  <span className='text-cyan-400'>{phone}</span></p>
                    <p className='my-[5px]'> <span className='font-bold'>- Phòng: </span> {item?.title}</p>
                    <p className='my-[5px]'> <span className='font-bold'>- {item?.address}</span> </p>
                    <p className='my-[5px]'> <span className='font-bold'>- Giá thuê theo tháng: </span>  {monthlyRateString}</p>
                    <p className='my-[5px]'> <span className='font-bold'>- Giá thuê theo ngày: </span>  {dailyRate.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</p>
                    <p className='my-[5px]'> <span className='font-bold'>- Diện tích: </span>  {item?.attributes?.acreage}</p>
                    <div className='my-4'>
                    ...
                    <p className='pb-[10px]'> - Tổng tiền cọc (50%): {totalCost.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</p>
                </div>
