...
const handleShowBookRoom = (item:any) => {
  ...
  return (
    <div className='w-full'>
                  ...
                  return (
                    <div key={index} className="flex my-[15px] border-b-[1px] border-gray-500 border-dashed pb-[15px] cursor-pointer">
                      ...
                      <div className='w-[20%] pt-[30px]'>
                        <Button text={'Đặt Phòng'} onClick={()=> handleShowBookRoom(item)} className={'w-full text-center bg-[#febb02] hover:bg-yellow-600 text-[#000000] font-bold hover:no-underline mb-[10px]'} icon={undefined} bgColor={undefined} textColor={undefined} px={undefined} />
      ...
      {isShowBookRoom && <PopupBookRoom handleSubmit={''} item={item} setIsShowBookRoom={setIsShowBookRoom} />}
...
// import React, { useEffect, useState } from 'react';
// import heat from '../../assets/imgs/ImgFavouritePage/Capture.png';
// import { useNavigate } from 'react-router-dom'; // Thay đổi import từ 'Link' sang 'useNavigate'
// import { FaChevronRight } from 'react-icons/fa';
// import { apiGetBookRoom, apiAddBookRoom, apiDeleteBookRoom } from '../../service/bookRoom';
// import { useSelector, useDispatch } from 'react-redux';
// import { apiGetPosts } from '../../service/post';
...
//     const [item, setItem] = useState({});
//     const dispatch = useDispatch();
//     const navigate = useNavigate(); // Sử dụng useNavigate thay thế cho Link
...
//     const handleDelete = async (idPost: any) => {
//         await apiDeleteBookRoom({ userId: id, postId: idPost });
//         getDataBookRoom();
//     };
...
//         // Điều hướng đến trang chi tiết phòng khi click vào item
//         navigate(`/chi-tiet/${formatVietnameseToString(item?.title.replaceAll('/', ''))}/${postId}`);
//     };

//     return (
//         <div className='w-full'>
//             <h1 className='text-[25px] font-bold my-[20px] cursor-pointer'>
//                 Số phòng đã lưu: {newFilteredData ? newFilteredData?.length : 0}{' '}
//             </h1>
//             <div className='w-[100%] flex mb-[40px] shadow-lg'>
//                 <div className={`w-full bg-white rounded-lg ${listFavorite.length > 0 ? '' : 'h-[250px]'}`}>
//                     {listFavorite.length > 0 ? (
...
//                                     </div>
//                                     <div className='w-[60%] mr-[10px]'>
//                                         <p className='text-[20px] text-[#ff0000] font-bold pr-[30px] mb-[10px]' onClick={() => handleItemClick(item.id)}>
//                                             {item?.title}
//                                         </p>
//                                         <p className='text-[16px] mb-[10px]'>{item?.address}</p>
//                                         <p>
...
//                                         <Button
//                                             text={'Đặt Phòng'}
//                                             onClick={() => handleShowBookRoom(item)}
//                                             className={'w-full text-center bg-[#febb02] hover:bg-yellow-600 text-[#000000] font-bold hover:no-underline mb-[10px]'}
//                                             icon={undefined}
//                                             bgColor={undefined}
//                                             textColor={undefined}
//                                             px={undefined}
...
//                                             className={'w-full text-center bg-red-500 hover:bg-red-700'}
//                                             icon={<FaTrashCan />}
//                                             bgColor={undefined}
//                                             textColor={undefined}
//                                             px={undefined}
