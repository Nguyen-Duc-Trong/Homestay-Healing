...
const handleChange = (item:any) => {
  ...
  const images1 = JSON.parse( item?.images?.image)

  // const images = item?.images?.image
  navigate(`/chi-tiet/${formatVietnameseToString(item?.title?.replaceAll('/', ''))}/${id}`, {
    state:{ images1, address, attributes, star, description, title, users, id, Overviews }
})
}   
            ...
            <div className="mx-[15px]">
              {
                newFilteredData?.map((item: any, index: any) => {
                  return (
                    <div onClick={() => handleChange(item)} key={index} className="flex my-[15px] border-b-[1px] border-gray-500 border-dashed pb-[15px] cursor-pointer">
