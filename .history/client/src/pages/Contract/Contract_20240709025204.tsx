...
const Contract = () => {
    const { id } = useSelector((state: any) => state.auth);
    const [dataContract, setDataContract] = useState([]);
    const [settledContracts, setSettledContracts] = useState<{ [key: string]: boolean }>({});
    const [amountToPay, setAmountToPay] = useState<{ [key: string]: number }>({});
    ...
    const handleSettlement = (contract: any) => {
        ...
        const amount = totalCost - deposit;
        setAmountToPay((prev) => ({ ...prev, [contract.contractId]: amount }));
        setSettledContracts((prev) => ({ ...prev, [contract.contractId]: true }));
    };

    const handleConfirm = (contract: any) => {
        // Xử lý logic xác nhận hợp đồng
        alert(`Xác nhận hợp đồng cho ${contract.contractId}`);
    };
    ...
    return (
        <div className='bg-white w-full shadow-lg mt-[25px]'>
            ...
            {dataContract?.length > 0 ?
                <div className="flex flex-wrap">
                    {dataContract?.map((item: any, index: any) => {
                        const isSettled = settledContracts[item.contractId];
                        return (
                            <div key={index} className="w-[47%] m-auto border-[1px] mt-0 p-[10px] mb-[15px]">
                                <p> - Họ tên người đặt: {item?.username}</p>
                                <p> - Số điện thoại: {item?.phone}</p>
                                ...
                                {!isSettled ? (
                                    ...
                                    <Button text={'Xác Nhận'} onClick={() => handleConfirm(item)} className={'w-full text-center bg-green-500 hover:no-underline'} icon={undefined} bgColor={undefined} textColor={undefined} px={undefined} />
                                )}
                            </div>
                        );
                    })}
