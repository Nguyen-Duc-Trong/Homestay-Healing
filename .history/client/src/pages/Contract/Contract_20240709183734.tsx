...
import PopupQR from './component/popupQR.tsx';
...
const handleShowQR = (item:any) => {
  ...
  return (
    <div className='bg-white w-full shadow-lg mt-[25px]'>
                    ...
                    return(
                        <div key={index} className="w-[47%] h-[400px] shadow-xl relative m-auto border-[1px] mt-0 mb-[15px] rounded-[8px]">
                            ...
                            <div className='absolute bottom-[0px] w-[100%]'>
                                {
                                    item?.accept ?
                                        <Button text={`Xác nhận`} onClick={()=> handleShowQR(item)} className={'w-full rounded-t-[0px] text-center  bg-green-500 hover:no-underline'} icon={undefined} bgColor={undefined} textColor={undefined} px={undefined} />
                                        :
                                        <Button text={'Thanh Lý Hợp Đồng'} onClick={() => handleSettlement(item)} className={'w-full rounded-t-[0px] text-[14px] font-bold text-center bg-[#febb02] hover:no-underline'} icon={undefined} bgColor={undefined} textColor={undefined} px={undefined} />
                                }
                            </div>
                        </div>
                    )
        ...
        {isShowQR && <PopupQR setIsShowQR={setIsShowQR} />}
    </div>
  )
}
