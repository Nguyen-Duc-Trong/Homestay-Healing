...
import { apiGetPublicProvinces } from '../../../service/app.js'
const Posting = () => {
...
const getData = async ()=>{
  ...
  const apiProvinces= await apiGetPublicProvinces()
  const data = apiPrices?.data?.response
  const dataProvinces = apiProvinces?.data?.response
  setMinMaxArr(dataProvinces)
}
useEffect(() => {
getData()
}, [])

const setPriceCode = (price, minMaxArr) => {
