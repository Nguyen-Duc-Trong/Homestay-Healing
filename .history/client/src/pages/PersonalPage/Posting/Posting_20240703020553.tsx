...
const handleSubmit = async () => {
  ...
  // checkMinMax(5000000, minMaxArr)
}
const getData = async ()=>{
  const apiPrices = await apiGetPrices()
  // const apiPrices = await apiGetPrices()
  const data = apiPrices?.data?.response
  const pricesArr :any = []
  data.forEach((item:any) => {
    pricesArr.push({ ...getMinMaxPrice(item?.value),code: item?.code } )
    return item
  });
  setMinMaxArr(pricesArr)
}
getData()
const checkMinMax = (price:any, minMaxArr:any) => {
  const numPrice = +price / 1000000;
  const matchingItem = minMaxArr.filter((item:any) => numPrice >= item.min && numPrice <= item.max);
  
  if (matchingItem) {
    return matchingItem.code;
  } else {
    return null; 
  }
}
