...
const handleSubmit = async () => {
  ...
  const code =  setPriceCode(5000000, minMaxArr)
  console.log(code);
...
const setPriceCode = (price:any, minMaxArr:any) => {
  setPayload({...payload, priceNumber: price.target.value })
  const numPrice = +price / 1000000;
  const priceRange = minMaxArr.find((item:any) => numPrice >= item.min && numPrice <= item.max);
  if(priceRange){
    console.log(priceRange.code);
    setPayload({...payload, priceCode : priceRange.code})
  }
  return priceRange ? priceRange.code : null;
}
console.log(minMaxArr);
