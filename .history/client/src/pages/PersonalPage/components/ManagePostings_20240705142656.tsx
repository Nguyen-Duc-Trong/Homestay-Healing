                    ...
                    posts.map((item:any, index:any)=>{
                      return (
                          <tr className='my-[10px]' key={index}>
                            <td  className=' border-r-[2px]  max-w-[150px]'>{item?.id}</td>
                            <td  className=' border-r-[2px]'><img className='w-[50px] h-[50px]' src={JSON.parse(item?.images?.image)[0]} alt="img" /></td>
                            <td  className=' border-r-[2px] max-w-[250px]'>{item?.title}</td>
                            <td  className=' border-r-[2px]'>{item?.attributes?.price}</td>
                            <td  className=' border-r-[2px]'><small>{item?.Overviews?.createdAt}</small> </td>
                            <td  className=' border-r-[2px]'><small>{item?.Overviews?.updatedAt}</small> </td>
                            <td  className=' border-r-[2px]'></td>
                            <td   className=' border-r-[2px]'><button className='w-[100%] h-[100%] bg-red-300'>Xóa</button> </td>
                            <td   className=' border-r-[2px]'><button className='w-[100%] h-[100%] bg-yellow-200'>Sửa</button> </td>
                          </tr>
                      )
                    })
                :
